...
def construct_batch(
    ...
    # to device
    batch_rho_coords = torch.from_numpy(batch_rho_coords).float().to(device)
    batch_theta_coords = torch.from_numpy(batch_theta_coords).float().to(device)
    batch_input_feat = torch.from_numpy(batch_input_feat).float().to(device)
    batch_mask = torch.from_numpy(batch_mask).long().to(device)
...
def compute_val_test_desc(
    model,  # todo substitude by a model
    idx,
    rho_wrt_center,
    theta_wrt_center,
    input_feat,
    ...
    for kk in range(num_batches):
        ...
        feed_dict = {
            "rho_coords": batch_rho_coords,
            "theta_coords": batch_theta_coords,
            "input_feat": batch_input_feat,
            "mask": batch_mask,
        }

        model.eval()
        desc, _, _ = model(**feed_dict)
...
def train_ppi_search(
    # the main part for training and logging
    ...
    print("Log dir: {}".format(out_dir))
    logfile = open(out_dir + "log_pth.txt", "w")
    logfile.write(
    ...
    for num_iter in tqdm(range(num_iterations)):
        ...
        if num_iter == 0:
            _, _, score = model(**feed_dict)
            score = score.detach().cpu().numpy()
            training_loss = 0
        else:
            optimizer.zero_grad()
            _, training_loss, score = model(**feed_dict)
            training_loss.backward()
            optimizer.step()
            training_loss = training_loss.detach().cpu().numpy()
            score = score.detach().cpu().numpy()
        ...
        if num_iter % num_iter_test == 0:
            ...
            if val_auc > best_val_auc:
                ...
                output_model = out_dir + "model_pth"
