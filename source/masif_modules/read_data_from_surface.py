...
def read_data_from_surface(ply_fn, params):
    ...
    n = len(mesh.vertices)  # at most 200?
    
    input_feat = np.zeros((n, params['max_shape_size'], 5))  # max_shape_size is a padding 
...
def compute_shape_complementarity(ply_fn1, ply_fn2, neigh1, neigh2, rho1, rho2, mask1, mask2, params):
    ...
    w = params['sc_w']
    int_cutoff = params['sc_interaction_cutoff']  # less than 1.5 are defined as ppi
    radius = params['sc_radius']
    num_rings = 10
    scales = np.arange(0, radius, radius/10)
    scales = np.append(scales, radius)

    v1 = mesh1.vertices  # vertices features
    v2 = mesh2.vertices
